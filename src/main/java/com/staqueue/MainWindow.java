...
public class MainWindow extends JPanel{
    ...
    private List<BFSNode> node = new ArrayList<>();
    ...
    private final ActionListener nodeButtonEvent = (ActionEvent e) -> {
        ...
        if (!button.isSelected()) {
            ...
            node.add(new BFSNode(nodeId, columns));
        }
        // If node is already selected then remove from the list
        else {
            button.setSelected(false);
            button.setBackground(Color.LIGHT_GRAY);
            node.remove(new BFSNode(nodeId, columns));
        }
    };
    ...
    private final ActionListener showMstEvent = (ActionEvent e) -> {
        ...
        // BFSAlgorithm prim = new BFSAlgorithm(node, adjacencyMatrix);
